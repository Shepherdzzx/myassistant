...
class ShellAssistant:
    def __init__(self, api_key: str, model: str = "qwen3-coder-plus",
        ...
        self.safe_commands = {
            ...
            "wc", "sort", "uniq", "diff", "mkdir", "rmdir", "cp", "mv",
            "rm", "touch", "chmod", "chown", "date", "cal", "bc", "man",
            "which", "whoami", "id", "ps", "top", "df", "du", "free",
            "ping", "curl", "wget", "git", "tar", "zip", "unzip", "gzip"
        }
    ...
    def chat(self, prompt: str, execute_shell: bool = False) -> str:
        """与模型进行交互，保持上下文记忆"""
        if prompt.strip().startswith('!') and execute_shell:
            command = prompt[1:].strip()
            output = self.execute_command(command)

            self.history.append({"role": "user", "content": f"Execute command: {command}"})
            self.history.append({"role": "assistant", "content": output})

            self._trim_history()
            self._save_context()

            # 对于命令输出，使用普通打印（通常是纯文本）
            print("Assistant: ", end="")
            print(output)
            return output
        ...
        # 使用rich渲染完整的Markdown响应
        if full_response and not prompt.strip().startswith('!'):
            # 检查是否包含Markdown特征
            if any(pattern in full_response for pattern in ['#', '**', '*', '```', '>']):
                print()  # 添加额外换行
                self._render_output(full_response)
                print()  # 添加额外换行
                ...
                try:
                    ...
                    if user_input.lower() == '/history':
                        ...
                        for idx, msg in enumerate(self.history):
                            ...
                            # 对于历史记录，使用普通显示
                            print(f"{idx+1}. {prefix}: {msg['content'][:200]}{'...' if len(msg['content']) > 200 else ''}")
                        print()
                        continue
